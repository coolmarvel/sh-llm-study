      ...
      <div className="grid gap-4 two-col" style={{ gridTemplateColumns: '420px 1fr' }}>
        <div className="card" style={{ padding: 8 }}>
          <div className="run-row label" style={{ cursor: 'default', height: 28 }}>
            <span>run</span><span>파라미터</span><span>step</span><span>best val</span><span>경과</span>
          </div>
          {runs.map((r) => (
            <div key={r.name} className={`run-row ${r.name === run?.name ? 'selected' : ''}`} onClick={() => setSelected(r.name)}>
              <span className="truncate"><span className="dot" />{r.name}</span>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="card">
            ...
            <div style={{ height: 340 }}>
              <ResponsiveContainer>
                <LineChart data={log} margin={{ top: 16, right: 16, bottom: 4, left: 0 }}>
                  ...
                  />
                  <Line type="monotone" dataKey="train_loss" name="train" stroke="var(--series-train)" strokeWidth={2} dot={false} isAnimationActive={false} />
                  <Line type="monotone" dataKey="val_loss" name="val" stroke="var(--series-val)" strokeWidth={2} dot={false} isAnimationActive={false} />
