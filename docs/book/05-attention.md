...
- 3장의 `(B, T, C)` 와 위치 임베딩, 4장의 `nn.Linear` 와 softmax.
- 행렬 곱의 shape 규칙: `(T, d) @ (d, T) → (T, T)`, `(T, T) @ (T, d) → (T, d)`. 이 장은 이 두 곱이 전부다.
- `transpose(-2, -1)`: 마지막 두 축을 바꾼다. `(B, T, d) → (B, d, T)`.

## 핵심 용어

| 용어 | 한 줄 정의 | 비유 |
|---|---|---|
| **어텐션(attention)** | 각 자리가 다른 자리들에 배분하는 가중치(합 1)로 값을 섞는 연산 | 회의에서 누구 말을 얼마나 들을지 정하기 |
| **셀프(self)** | 질문·꼬리표·값이 모두 같은 시퀀스에서 나온다 | 문장이 자기 자신을 참조 |
| **Q · K · V** | 입력을 세 역할로 투영한 벡터. 질문 · 꼬리표 · 내용 | `Map` 의 조회 키 · 저장 키 · 값 |
| **점수(scores)** | Q·Kᵀ/√d, 자리 i 가 j 를 보고 싶은 정도의 원점수 `(T, T)` | 관련도 점수표 |
| **가중치(weights)** | 점수의 softmax. 행마다 합 1 | 시선 배분 비율 |
| **인과 마스크(causal mask)** | 미래 자리의 점수를 −∞ 로 막기 | 시험지에서 뒷장을 가리기 |
| **헤드(head)** | 독립적인 Q·K·V 세트 하나. 여러 개를 병렬로 둔다 | 각자 다른 관점의 검토자 |
| **head_size** | 헤드 하나의 벡터 길이 = C / n_head | 검토자 한 명이 보는 항목 수 |
...
T=3, d=2. 투영은 끝났다고 치고 Q, K, V 를 직접 준다 (행 = 자리).
...
**점수** = Q Kᵀ / √2. 자리 2 (Q=[1,1]) 의 행: [1·1+1·0, 1·0+1·1, 1·1+1·1] / 1.414 = [0.707, 0.707, 1.414].
**마스크**: 자리 2 는 0, 1, 2 를 다 봐도 된다(마지막 자리). 자리 0 은 [자기 자신만], 자리 1 은 [0, 1].
**softmax** (자리 2): exp → [2.03, 2.03, 4.11], 합 8.17 → 가중치 **[0.25, 0.25, 0.50]**. 자기 자신을 절반, 앞 둘을 1/4 씩.
**출력** (자리 2) = 0.25·[1,0] + 0.25·[0,1] + 0.5·[2,2] = **[1.25, 1.25]**.
자리 0 의 출력은 가중치 [1, 0, 0] 이므로 V[0] = [1, 0] 그대로다. 첫 토큰은 볼 것이 자기뿐이다.

Q 의 방향이 K 와 맞을수록 점수가 크다. 자리 2 의 Q=[1,1] 은 K=[1,1] 과 가장 잘 맞아 자기 자신을 가장 많이 봤다.
학습이 하는 일은 `Wq, Wk, Wv` 를 조정해 **"어떤 질문이 어떤 꼬리표와 맞아야 다음 토큰을 잘 맞히는가"** 를 찾는 것이다.

같은 계산을 코드로:

```python
q = k = torch.tensor([[1., 0.], [0., 1.], [1., 1.]]); v = torch.tensor([[1., 0.], [0., 1.], [2., 2.]])
out, w = attention(q, k, v, causal_mask(3))
w[2]      # tensor([0.25, 0.25, 0.50])
out[2]    # tensor([1.25, 1.25])
```
...
| 이 장 | 자바로 치면 |
|---|---|
| `q @ k.transpose(-2, -1)` | `for i, for j: scores[i][j] = dot(q[i], k[j])` 이중 루프. 배치·헤드 축은 그 바깥의 루프 두 겹 |
| `masked_fill(~mask, -inf)` | `if (j > i) scores[i][j] = Double.NEGATIVE_INFINITY` |
| `softmax(dim=-1)` | 행마다 exp 후 합으로 나누기. −∞ 는 exp 하면 0 |
| `weights @ v` | `out[i] = Σ_j weights[i][j] * v[j]` 가중 합 |
| `view(B, T, n_head, head_size).transpose(1, 2)` | `double[B][T][C]` 를 `double[B][H][T][C/H]` 로 재해석. 복사 없이 인덱스 계산만 바꾼다 |
| `register_buffer("mask", ...)` | `static final` 상수 배열. 저장·로드는 되지만 학습 대상은 아님 |
| `last_weights` | 디버깅용 필드. 마지막 호출의 중간값을 남긴다 |

### 흔한 오해

- **"어텐션은 문법을 안다"**, 아니다. Q·K 내적이 큰 자리를 볼 뿐이고, 학습이 "그렇게 보면 loss 가 준다"는 방향으로 Wq·Wk 를 밀었을 뿐이다. 6장 어텐션 맵의 "직전 토큰 헤드"는 그 결과다.
- **"√d 는 사소한 상수"**, d=512 에서 점수의 표준편차는 22.6. softmax 에 넣으면 최대 칸이 1.0 이 되고 기울기가 0 이 된다. 학습이 아예 안 된다.
- **"마스크는 학습 때만 필요하다"**, 생성 때도 필요하다. 미래 자리는 아직 없으니 자연히 못 보지만, 학습 때 T 개 자리를 한 번에 계산하려면 마스크가 있어야 "자리 i 는 i 까지만" 이 지켜진다.
- **"헤드가 많을수록 좋다"**. C 를 나눠 쓰므로 헤드가 많으면 헤드 하나가 얇아진다. GPT-2 는 C/n_head = 64 를 유지한다. small-cpu 는 256/8 = 32.
...
1. 위 예시에서 자리 1 의 가중치와 출력을 손으로 구하라. (Q=[0,1], K 0·1 만 본다)
2. Q·K·V 세 투영이 각각 하는 일을 `HashMap` 조회에 빗대 설명하라. 어텐션이 `HashMap` 과 다른 점은?
3. 마스크 없이 순서를 섞으면 출력도 그대로 섞이는 이유는? 그러면 순서 정보는 어디서 오나?
4. `(B, T, C)` 가 멀티헤드 안에서 `(B, H, T, C/H)` 로 바뀌었다가 다시 `(B, T, C)` 로 돌아오는 과정을 shape 로 써 보라.
5. 어텐션 층의 파라미터가 약 4C² 인 이유를 부품별로 나눠 설명하라.
6. 5.5 의 비교에서 어텐션 모델이 같은 스텝 수에 훨씬 좋았던 두 가지 이유는?
